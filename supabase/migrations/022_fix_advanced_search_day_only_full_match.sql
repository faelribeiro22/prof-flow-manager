-- Migration: Require full day coverage for day-only advanced search
--
-- Problem:
-- When filtering by p_day_of_week_list only (without p_hour_list or p_time_ranges),
-- the function matched teachers with availability on ANY selected day. This violates
-- the rule that all selected criteria must be satisfied.
--
-- Fix:
-- For day-only searches, require that each selected day has at least one free slot.
...
BEGIN
  ...
  WHERE
    ...
    AND (
      ...
      OR (
        -- Day-only filter: require at least one free slot on every selected day
...
COMMENT ON FUNCTION public.search_teachers_advanced(
  ...
  UUID[],
  TEXT
) IS 'Busca avancada de professores com cobertura completa para intervalos e dias selecionados.';
